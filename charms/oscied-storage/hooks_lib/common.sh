...
hook_install()
{
  ...
  pecho 'Install GlusterFS Server'
  eval $install glusterfs-server nfs-common || xecho 'Unable to install packages' 3

  pecho "Create medias volume $MEDIAS_VOLUME"
  ip=$(unit-get private-address)
  gluster volume create "$MEDIAS_VOLUME" "$ip:/$MEDIAS_BRICK" || \
    xecho "Unable to create medias volume $MEDIAS_VOLUME on brick $ip:/$MEDIAS_BRICK" 4

  gluster volume start "$MEDIAS_VOLUME" || xecho "Unable to start volume $MEDIAS_VOLUME" 5
  gluster volume info
