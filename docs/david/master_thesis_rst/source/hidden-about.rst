.. include:: common.rst

.. |MOTTO| replace:: It's not a bug - it's an undocumented feature.
.. _MOTTO: http://en.wikipedia.org/wiki/Undocumented_feature#cite_note-2

.. only:: html

    About
    *****

.. raw:: html

    <h2>The Author</h2>

.. only:: html

    :author: David Fischer
    :address: Chemin de Saule 57, 1233 Bernex
    :contact: :email:`david.fischer.ch@gmail.com`
    :motto: |MOTTO|_

.. raw:: latex

    \chapter*{About}
    \section*{The Author}

.. only:: latex

    :author: David Fischer
    :address: Chemin de Saule 57, 1233 Bernex
    :contact: david.fischer.ch@gmail.com
    :motto: |MOTTO|_

.. raw:: html

    <h2>The Thesis</h2>

.. raw:: latex

    \section*{The Thesis}

:title: |DASH| Distribution Platform
:school: |hepia|_
:organization: |MSE|_
:domain: Multimedia & Informations Technologies
:responsible: Prof. Andrés Revuelta
:date: 8th February 2012

.. raw:: html

    <h2>The Project</h2>

.. raw:: latex

    \section*{The Project}

:name: |OSCIED|
:organization: |EBU|_
:project leader: Bram Tullemans
:main developer: David Fischer
:license: |GPL|_
:release: Basic V1.0
:revision: 68da8c1ba0d6da1e31e3bcfaecbf27f2ba0bfd89
