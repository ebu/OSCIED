...
@app.route('/transform/profile/id/<id>', methods=['PATCH', 'PUT'])
def api_transform_profile_id_patch(id):
    """
    Update a transform profile.

    .. warning::

        All fields can be updated, maybe not a good idea (??)

   **Example request**:

    .. sourcecode:: http

        PUT /transform/profile/id/c316ff1a-74f8-11e2-82d4-3085a9accd33 HTTP/1.1
        Host: somewhere.com
    ...
    **Example response**:
    ...
    try:
        ...
        if 'encoder_string' in data:
            profile.encoder_string = data['encoder_string']
        orchestra.save_transform_profile(profile)
    except (ValueError, TypeError) as error:
        abort(400, str(error))
    except KeyError as error:
        abort(400, 'Key ' + str(error) + ' not found.')
    return ok_200('The transform profile "' + old_title + '" has been updated.', False)
